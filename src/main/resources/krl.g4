...
//程序入口
start
    : NEWLINE* krlControlBlock moduleData EOF   # dataFile  // 数据文件
    | NEWLINE* krlControlBlock moduleSource EOF   # sourceFile // 代码文件
    ;
...
variableDeclaration
    : modifier* DECL? modifier* typeName expression (',' expression)*

    //信号变量的定义方式，比如:
    // SIGNAL START_PROCESS $OUT[7]
    // GLOBAL SIGNAL GI_SpeedFromPLC $IN[20]  TO $IN[22]
    | modifier*  SIGNAL modifier* variableName variableName (TO variableName)?
    ;
...
statement
    ...
    | (PTP | SPTP | PTP_REL) expression (C_DIS | C_ORI | C_VEL|C_SPL|C_PTP)* (WITH expression(',' expression)*)? (C_DIS | C_ORI | C_VEL | C_SPL)* NEWLINE # moveStatement

    //LIN 运动到命名点。C_DIS、C_ORI、C_VEL 分别表示是否采用距离、姿态、速度百分比来进行过渡约束。
    | (LIN | SLIN) expression  (C_DIS | C_ORI | C_VEL | C_SPL|C_PTP)* (WITH expression(',' expression)*)? (C_DIS | C_ORI | C_VEL | C_SPL)* NEWLINE # moveStatement

    //LIN_REL 运动到相对命名点。enumLiteral表示是否有枚举关键字，比如 #FINE #COARSE等。
    | LIN_REL expression (C_DIS | C_ORI | C_VEL | C_SPL|C_PTP )* enumLiteral? (WITH expression(',' expression)*)? (C_DIS | C_ORI | C_VEL | C_SPL)* NEWLINE # moveStatement

    /*
        圆弧运动。通过 两个点加 角度确定圆弧轨迹。比如
        CIRC {X 5,Y 0, Z 9.2},{X 12.3,Y 0,Z -5.3,A 9.2,B -5,C 20}, CA 260 C_ORI
    */
    | (SCIRC | CIRC | CIRC_REL) expression ',' expression (',' krlIdentifier expression)? ( C_DIS | C_ORI| C_VEL |C_SPL)*  (WITH expression(',' expression)*)? ( C_DIS | C_ORI| C_VEL |C_SPL)* NEWLINE # moveStatement
...
analogOutputStatement
    : ANOUT ON expression (DELAY '=' literal)? (MINIMUM '=' literal)? (MAXIMUM '=' literal)?
    | ANOUT OFF expression
    ;

switchBlockStatementGroups
    : (caseLabel NEWLINE+ statementList)+ (defaultLabel NEWLINE+ statementList)?
    ;
...
primary
    : '(' expression ')'     // 小括号包裹的内容
    | literal               // 字面量
    | variableName          // 变量名，包括变量与数组。$IN[5]、$OUT[8]、$ANIN[10]、$ANOUT[11]等包含在此类中
    | variableName ('.' variableName)+  // 通过`.` 访问结构体成员
    | variableName '('    (expression (',' expression?)*)?   ')'          // 函数调用（参数可带也可不带）
    ;
